...
export class Request extends InteractsWithHeaders {
     ...
     */
    getClientIp(prependCustomHeaders: string[] = []): string | undefined {
        return (
            this.fromCustomHeaders(prependCustomHeaders) ??
            this.fromHeaders() ??
            this.fromSocket() ??
            this.fromConnection() ??
            this.fromInfo() ??
    ...
    fromCustomHeaders(prependCustomHeaders: string[] = []): string | undefined {
        for (const header of prependCustomHeaders) {
            if (this.hasIpInHeader(header)) {
                return this.ipInHeader(header);
            }
        }
